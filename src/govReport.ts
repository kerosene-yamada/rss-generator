...
const PROMPTS: Record<string, { label: string; text: string }> = {
  shougaiji: {
    ...
    text: `障害児通所支援事業における報酬改定や制度・運営に関する最新動向について、厚生労働省およびこども家庭庁等が公開する検討会・審議会の議事録・報告書・公式通知・資料から、直近2週間以内の新着情報があればまとめてレポートしてください。`,
  },
  ahaki: {
    label: 'あん摩マッサージ 最新動向レポート',
    text: `あん摩マッサージにおける報酬改定や制度・運営に関する最新動向について、厚生労働省等が公開する検討会・審議会の議事録・報告書・公式通知・資料から、直近2週間以内の新着情報があればまとめてレポートしてください。`,
  },
};
